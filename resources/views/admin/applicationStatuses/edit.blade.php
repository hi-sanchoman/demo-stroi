...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.applicationStatus.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.application-statuses.update", [$applicationStatus->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('application') ? 'is-invalid' : '' }}" name="application_id" id="application_id" required>
                    @foreach($applications as $id => $entry)
                        <option value="{{ $id }}" {{ (old('application_id') ? old('application_id') : $applicationStatus->application->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('application_path') ? 'is-invalid' : '' }}" name="application_path_id" id="application_path_id" required>
                    @foreach($application_paths as $id => $entry)
                        <option value="{{ $id }}" {{ (old('application_path_id') ? old('application_path_id') : $applicationStatus->application_path->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\ApplicationStatus::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $applicationStatus->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('declined_reason') ? 'is-invalid' : '' }}" name="declined_reason" id="declined_reason" required>{{ old('declined_reason', $applicationStatus->declined_reason) }}</textarea>
