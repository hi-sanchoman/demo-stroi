    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.applications.update", [$application->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('construction') ? 'is-invalid' : '' }}" name="construction_id" id="construction_id" required>
                    @foreach($constructions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('construction_id') ? old('construction_id') : $application->construction->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('issued_at') ? 'is-invalid' : '' }}" type="text" name="issued_at" id="issued_at" value="{{ old('issued_at', $application->issued_at) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('kind') ? 'is-invalid' : '' }}" name="kind" id="kind" required>
                    ...
                    @foreach(App\Models\Application::KIND_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('kind', $application->kind) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('kind'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_urgent') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_urgent" id="is_urgent" value="1" {{ $application->is_urgent || old('is_urgent', 0) === 1 ? 'checked' : '' }}>
